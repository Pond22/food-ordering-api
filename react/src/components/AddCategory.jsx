...
const AddCategory = () => {
  ...
  const handleCategorySubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setSuccessMessage(
          `หมวดหมู่ "${response.data.nameEn}" ถูกเพิ่มเรียบร้อยแล้ว!`
        )
        setCategoryNameTH('')
        setCategoryNameEN('')
        setCategoryNameCH('')
  ...
  const handleEditCategorySubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setSuccessMessage(
          `หมวดหมู่ "${response.data.nameEn}" ถูกแก้ไขเรียบร้อยแล้ว!`
        )
        fetchCategories() // รีเฟรชข้อมูลหลังจากแก้ไขหมวดหมู่
        setShowEditModal(false) // ปิดป็อปอัพ
        setEditingCategory(null)
      }
    } catch (error) {
      if (error.response) {
        setErrorMessage('เกิดข้อผิดพลาดในการแก้ไขหมวดหมู่')
      } else {
        setErrorMessage('ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้')
      }
    } finally {
  ...
  const handleAddCategoryClick = () => {
    setCategoryNameTH('') // เคลียร์ชื่อหมวดหมู่ภาษาไทย
    setCategoryNameEN('') // เคลียร์ชื่อหมวดหมู่ภาษาอังกฤษ
    setCategoryNameCH('') // เคลียร์ชื่อหมวดหมู่ภาษาจีน
    setShowModal(true) // เปิด Modal สำหรับเพิ่มหมวดหมู่
  }

  // ฟังก์ชันสำหรับการคลิกแก้ไขหมวดหมู่
  const handleEditCategory = (category) => {
    setCategoryNameTH(category.Name)
    setCategoryNameCH(category.NameCh)
    setCategoryNameEN(category.NameEn)
    setEditingCategory(category) // เซ็ตหมวดหมู่ที่กำลังจะแก้ไข
    setShowEditModal(true) // เปิดป็อปอัพแก้ไข
  }
  ...
  const handleRestoreCategory = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setSuccessMessage(
          `หมวดหมู่ "${response.data.category.Name}" กู้คืนเรียบร้อยแล้ว!`
        ) // แสดงข้อความสำเร็จ
        fetchDeletedCategories() // รีเฟรชข้อมูลหมวดหมู่ที่ถูกลบ
  ...
  // ฟังก์ชันสำหรับการเรียงหมวดหมู่ตาม ID
  const sortedCategories = filteredCategories.sort((a, b) => {
    if (sortAscending) {
      return a.ID - b.ID // เรียงจากน้อยไปหามาก
    } else {
      return b.ID - a.ID // เรียงจากมากไปหาน้อย
    }
  })

  return (
    <div className=" mx-auto p-6 bg-gray-50 flex items-center justify-center ">
        ...
        {activeTab === 'addCat' && (
          <div className="bg-gray-50 w-full p-8 rounded-lg">
            <div className="relative flex-grow flex justify-between">
              ...
              <button
                onClick={handleAddCategoryClick} // เรียกใช้ฟังก์ชัน handleAddCategoryClick
            ...
            <table className="min-w-full table-auto border-collapse border border-gray-200">
              ...
              <tbody>
                {deletedCategories.map &&
                  filteredCategoriesDel.map((category) => (
                    <tr key={category.ID} className="border-b hover:bg-gray-50">
                      <td className="p-4 text-center">{category.ID}</td>
                      <td className="p-4 text-center">
                        {category.Name || 'ไม่มี'}
                      ...
                      <td className="p-4 text-center">
                        {category.NameCh || 'ไม่มี'}
                      </td>
                      <td className="p-4 text-center">
                        {category.NameEn || 'ไม่มี'}
                      </td>
                      <td className="p-4 text-center">
                        <button
                          onClick={() => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-8 rounded-lg shadow-lg w-96">
            ...
            <p className="text-center mb-4">
              คุณต้องการกู้คืนหมวดหมู่ "{categoryToRestore?.Name}"
              และเมนูทั้งหมดในหมวดหมู่นี้หรือไม่?
            </p>
            ...
            <form onSubmit={handleCategorySubmit} className="space-y-4">
              ...
              <div>
                <button
                  ...
                  } text-white font-medium py-2 px-4 rounded-lg hover:bg-blue-600 transition duration-200`}
                  disabled={loading}
                >
            ...
            <form onSubmit={handleEditCategorySubmit} className="space-y-4">
              ...
              <div>
                <button
                  ...
                  disabled={loading}
                >
                  {loading ? 'กำลังอัพเดต...' : 'อัพเดตหมวดหมู่'}
                </button>
              </div>
            </form>
          </div>
